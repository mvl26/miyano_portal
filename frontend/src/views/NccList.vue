...
const error = ref('')
// Brief 2026-08-15 (phân trang) — truyền `limit` nên kho_ncc_list trả dạng
// {rows, tong} (nhánh phân trang, KHÁC nhánh dropdown không truyền limit).
const trang = ref(1)
const soDong = ref(20)
const tong = ref(0)
...
async function load() {
  ...
  try {
    ...
    const res = await api.callKho('kho_ncc_list', {
      ca_inactive: 1,
