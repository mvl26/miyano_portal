...
| 2–5 | *(xem lộ trình)* | ⬜ | |
| ? | Rò rỉ dòng chi tiết chứng từ qua `frappe.client.get_list`/`get` trên `Sales Order Item` / `Delivery Note Item` / `Sales Invoice Item` | ⬜ | Phát hiện khi review NG-37 (2026-08-12), **KHÔNG** thuộc phạm vi NG-37 — NG-37 chỉ vá `search_link`/`search_widget`. Cần mã số riêng + quyết định phương án (xem §4 mục NG-37, "Phát hiện thêm"). Tiền lệ: `supplycore/supplycore/hooks.py:170-186` |
