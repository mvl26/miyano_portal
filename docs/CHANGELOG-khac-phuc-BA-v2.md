...
| Đợt đang chạy | **Đợt 1 — Chặn máu (P0)** — Task 0, 1, 1b, 1c, 1d xong (NG-37e đóng kèm); kế tiếp Task 2 (NG-12). Còn mở trong đợt: NG-37f (ghi nhận), NG-37g (cuối đợt) |
...
| 1 | NG-37d rò rỉ/kết quả sai qua `frappe.client.get_value` · `validate_link` · `has_permission` (docname dạng chuỗi) trên MỌI doctype con | ✅ Task 1d (2026-08-12) | Ba lớp bọc `client_get_value` / `client_validate_link` / `client_has_permission` trong `search_guard.py`, đăng ký qua `override_whitelisted_methods`, gate `is_table` giống NG-37b. RED gate in-process đã tái lập cả ba lỗ trước khi vá. Xem §4. *(Ghi chép phát hiện gốc, giữ nguyên:)* Phát hiện khi quét Step 7 của NG-37b, KHÔNG thuộc phạm vi NG-37b lẫn NG-37c (route `/api/method/frappe.client.get_value`\|`validate_link`\|`has_permission` — không phải REST resource/document nên `rest_guard.py` không chạm tới; không có entry trong `override_whitelisted_methods` nên `search_guard.py` cũng không chạm tới). **Re-probe HTTP thật, 2026-08-12, phiên `bvbm@demo.miyano`, doctype `Payment Schedule` (không phải ba doctype PoC gốc — cố ý chọn khác để xác nhận đây là lỗ theo TRỤC HÀM, không phụ thuộc doctype):** `GET /api/method/frappe.client.get_value?doctype=Payment+Schedule&parent=Sales+Invoice&fieldname=["parent","parenttype","payment_amount","outstanding"]` → `{"message":{"parent":"ACC-SINV-2026-00001",...,"outstanding":13900000.0}}` — dòng của khách KHÁC. **Chi tiết dễ hiểu lầm:** gọi KHÔNG kèm `parent=` thì `get_value`/`get` tự ném `PermissionError` (vì `check_parent_permission(None, doctype)` luôn `raise` khi `parent` rỗng — `db_query.py:1305-1318`) — trông giống như "đã chặn", nhưng chỉ cần client thêm đúng `parent=<parenttype thật>` là lọt hoàn toàn; đây KHÔNG phải phòng thủ, chỉ là tác dụng phụ tình cờ. `has_permission` re-probe: `GET /api/method/frappe.client.has_permission?doctype=Sales+Order+Item&docname=<dòng khách khác>&perm_type=read` → `{"has_permission": true}` (oracle sai, không lộ field nhưng xác nhận quyền sai). `validate_link` re-probe: `GET /api/method/frappe.client.validate_link?doctype=Payment+Schedule&docname=<dòng khách khác>` → trả `{"name": ...}` (không ném lỗi) thay vì `PermissionError`. Cả ba đều CHƯA sửa trong Task 1c (đúng brief: không lặng lẽ mở rộng phạm vi) — cần một task riêng, có thể port tiếp mẫu `search_guard.client_get_list`/`client_get` (đăng ký qua `override_whitelisted_methods`, KHÔNG cần `before_request` vì route này định tuyến bằng chuỗi tên) cho ba hàm này. |
| 1 | NG-37e cùng cơ chế có thể áp dụng cho `Blanket Order Item` | ✅ **đã đóng sẵn, nay xác nhận bằng probe** (Task 1d) | Không cần vá riêng: gate `frappe.is_table()` của NG-37b round 2 (và NG-37d) không liệt kê tên nên đã phủ doctype này từ trước — đúng lý do đã bỏ deny-list theo tên. Probe 2026-08-12, phiên `bvbm@demo.miyano`: `is_table("Blanket Order Item")` → `True`; meta có trường tiền `rate`; bảng có 41 dòng thật; `client_get_list(..., parent="Blanket Order")` → `[]`; `client_get_value(...)` → `PermissionError`. Mức độ nghiêm trọng nếu còn hở: có `rate` nhưng không có `amount`/`outstanding`, tức thấp hơn `Payment Schedule`. *(Nghi vấn gốc, nay đã trả lời: cha `Blanket Order` có `blanket_query`/`generic_has_permission` giống ba doctype con NG-37b nên lý thuyết cùng một lỗ `check_parent_permission` doctype-level — đúng về cơ chế, nhưng gate `is_table` đã phủ trước.)* |
...
| 1 | NG-37g `before_request` chạy TRƯỚC `validate_auth()` — auth bằng API key bỏ qua được NG-37c | ⬜ **còn mở, phát hiện khi review Task 1c** | `frappe/app.py::application()` gọi `init_request(request)` (dòng 105, vòng lặp `before_request` hook nằm TRONG hàm này, dòng 208-209) RỒI MỚI gọi `validate_auth()` (dòng 107) — hàm phân giải `Authorization: token key:secret`. Với request dùng API key (không có cookie `sid`), tại thời điểm `chan_rest_doctype_con()` chạy, `frappe.session.user` vẫn là `"Guest"` (chưa được `validate_auth()` gán) → nhánh `if not user or user == "Guest": return` của hook tự thoát sớm → KHÔNG chặn. **Xác nhận bằng probe HTTP thật**, 2026-08-12: cấp API key cho `bvbm@demo.miyano` bằng `frappe.core.doctype.user.user.generate_keys()` (chạy dưới `Administrator` trong `bench console`, vì hàm này tự giới hạn `frappe.only_for("System Manager")` — Website User portal KHÔNG tự cấp được key cho chính mình qua bất kỳ route công khai nào), rồi gọi `GET /api/v2/document/Payment%20Schedule` với header `Authorization: token <key>:<secret>` (không gửi cookie) → trả đầy đủ `outstanding`/`payment_amount` của khách khác, HTTP 200 — NG-37c bị bỏ qua hoàn toàn. Đã xoá `api_key`/`api_secret` test khỏi `bvbm@demo.miyano` ngay sau probe. **Mức độ rủi ro hiện tại: TIỀM ẨN, chưa khai thác được trong thực tế** — trước probe, KHÔNG có Website User nào trên `erptest.local` có `api_key` (xác nhận bằng `frappe.get_all("User", filters={"user_type":"Website User","api_key":["is","set"]})` → rỗng), và SPA (`frontend/src/api.js`) chỉ dùng cookie `sid`, không có route nào cấp API key cho khách cổng. Nghĩa là lỗ này chỉ khai thác được NẾU một System Manager (nhân viên Miyano) cấp API key cho một tài khoản Website User vì lý do khác (vd. tích hợp) — không phải luồng nghiệp vụ portal hiện tại. **Kế thừa từ mẫu `supplycore::portal_block_rest_child`** (docstring gốc chỉ nhắc `HTTPRequest()` resume cookie, không hề nhắc `validate_auth()`/API key) — không phải lỗi riêng của Task 1c, cùng một khoảng trống kiến trúc của `before_request` nói chung khi dùng để chặn theo user. Hướng sửa khả dĩ cho người làm task này: hoặc chuyển gate sang `auth_hooks`/kiểm tra lại trong chính route handler, hoặc gọi `validate_auth()` sớm hơn — CẢ HAI đều ngoài phạm vi Task 1c (đổi thứ tự framework hoặc thêm cơ chế mới), không tự sửa ở đây. |
...
### NG-37d · Rò rỉ qua `client.get_value` / `validate_link` / `has_permission` — 2026-08-12 · commit 6a684bd
**Trước:** Ba hàm này cùng lỗ `check_parent_permission()` với `get_list`/`get`, nhưng KHÔNG được NG-37b chạm tới vì chúng gọi lẫn nhau bằng tham chiếu NỘI BỘ trong `frappe/client.py` (`validate_link` → `get_value` → `get_list` cùng file), không qua `override_whitelisted_method()`. RED gate in-process 2026-08-12 (phiên `bvbm@demo.miyano`, dòng `Sales Order Item` thuộc đơn khách khác) tái lập đủ cả ba: `get_value` KHÔNG ném `PermissionError`, `validate_link` KHÔNG ném `PermissionError`, `has_permission` trả `True`.
**Sau:** Ba lớp bọc `client_get_value` / `client_validate_link` / `client_has_permission`, gate `_la_khach_cong() and frappe.is_table(doctype)` giống hệt NG-37b (theo THUỘC TÍNH, không liệt kê tên). Hai hàm đọc dữ liệu ném `PermissionError` tiếng Việt; `has_permission` là oracle trả dict nên trả câu trả lời ĐÚNG `{"has_permission": False}` thay vì ném — người gọi hợp lệ vẫn nhận đúng kiểu. Doctype không phải bảng con, và mọi người dùng không phải Website User, uỷ quyền nguyên trạng cho bản gốc.
**Đụng vào:** `miyano_portal/search_guard.py` (thêm khối `NG-37d` + 3 hàm) · `miyano_portal/hooks.py` (thêm 3 khoá vào `override_whitelisted_methods` đã mở sẵn từ NG-37) · `miyano_portal/tests/test_client_value_guard.py` (mới)
**Phá vỡ:** Không. SPA không gọi `frappe.client.*` (đã grep từ NG-37b, không đổi). Desk không đổi — có test riêng với `sales_user@demo.miyano` (System User thật, không phải Administrator).
**Test:** `miyano_portal.tests.test_client_value_guard` — 11 test: chặn cả ba hàm cho Website User, chặn cả dòng của CHÍNH khách gọi (chặn thẳng, không lọc), doctype cha vẫn uỷ quyền và lọc đúng, Desk user dùng được cả ba, hooks đăng ký đủ ba khoá, và dispatch thật (`frappe.override_whitelisted_method`) resolve đúng ba tên. Toàn suite **379 OK**.
**Cảnh báo chồng lấn:** `override_whitelisted_methods` nay có 7 khoá. Ai thêm override sau này thì thêm khoá vào cùng dict, đừng khai lại biến.
