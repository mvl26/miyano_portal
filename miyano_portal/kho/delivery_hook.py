...
def _tu_delivery_note(dn) -> str | None:
	...
	for row in dn.items:
		for so_lo, han_su_dung, so_luong in _lo_cua_dong(row):
			if so_luong <= 0:
				continue
			dong.append({
				"vat_tu": _vat_tu_trong_kho(kho, row, cache_vat_tu),
				"so_lo": so_lo,
	...
	return _tao_phieu(dn, kho, dong)


def _tao_phieu(dn, kho: str, dong: list[dict]) -> str:
...
# ------------------------------------------------------------------ vật tư kho
def _vat_tu_trong_kho(kho: str, row, cache: dict[str, str]) -> str:
	...
	if not name:
		...
		vt = frappe.get_doc({
			...
			"dvt": row.get("uom") or item.stock_uom or "Cái",
			"item_code": item_code,
		})
